...
def set_global_style():
    ...
    style.configure("Hint.TLabel", font=("微软雅黑", 9), foreground="#666666", background="#F5F5F5")
...
def search_file():
    ...
    try:
        ...
        result_tree.current_df = result_df
        result_tree.current_file = selected_file

    except Exception as e:
        showerror("错误", f"查询失败: {str(e)}")


def copy_cell_content(tree, event):
    ...
    col_idx = int(column.replace('#', '')) - 1  # 转换为0-based索引
    ...
    if hasattr(tree, 'current_file') and tree.current_file == '事件.xlsx' and col_name == 'inner_name':
        pass
    elif tree == command_tree and col_name == 'command':
        pass
    else:
        return
    ...
    try:
        ...
        showerror("错误", f"复制失败: {str(e)}")


def show_context_menu(tree, event):
    region = tree.identify_region(event.x, event.y)
    if region != "cell":
        return
    ...
    if can_copy:
        ...
        menu.add_command(label="复制", command=lambda: copy_cell_content(tree, event))
...
command_label.pack(anchor=tk.W, pady=(0, 4))

copy_hint_label = ttk.Label(left_frame, text="command 列与事件 inner_name 列可右键复制", style="Hint.TLabel")
copy_hint_label.pack(anchor=tk.W, pady=(0, 8))
...
command_tree.bind("<Button-3>", lambda e: show_context_menu(command_tree, e))
...
search_label = ttk.Label(top_right_frame, text="查询内容（输入all/*显示全部）",
                         style="Title.TLabel")
...
result_tree.bind("<Button-3>", lambda e: show_context_menu(result_tree, e))
